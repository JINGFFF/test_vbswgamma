...
class JetUserData : public edm::EDProducer {
	...
	private:
		...
		//void produce_jec_source( edm::Event &, const edm::EventSetup & );
		...
		std::vector<std::string> offsetCorrLabel_;
	        std::vector<std::string> jecAK4chs_source_;
	        std::string jecAK4chs_Uncfile_;
                //std::vector<std::string> jecAK4chs_source_;
		//std::vector<std::string> jec_source_jetCorrLabel_;
		//std::vector<std::string> jec_source_jecAK4chsLabels_;
		//std::vector<std::string> jec_source_offsetCorrLabel_;
...
JetUserData::JetUserData(const edm::ParameterSet& iConfig) :
	...
	//jec_source_jecAK4chsLabels_    (iConfig.getParameter<std::vector<std::string>>("jec_source_jecAK4chsPayloadNames_jetUserdata")),

        jecAK4chs_source_       (iConfig.getParameter<std::vector<std::string>>("jecAK4chs_sources")),
        jecAK4chs_Uncfile_  (iConfig.getParameter<std::string>("jecAK4chs_Uncfile")),
...

{
	...
	//jec_source_jetCorrLabel_ = jec_source_jecAK4chsLabels_;
	//jec_source_offsetCorrLabel_.push_back(jec_source_jetCorrLabel_[0]);
	//////// Meng 2017/5/8
...
void JetUserData::produce( edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    //string jec_source[11] = {"Absolute", "Absolute_2018", "BBEC1", "BBEC1_2018", "EC2", "EC2_2018", "FlavorQCD", "HF", "HF_2018", "RelativeBal", "RelativeSample_2018"};//, "Total"};
    //const char* srcnames[11] = {"Absolute", "Absolute_2018", "BBEC1", "BBEC1_2018", "EC2", "EC2_2018", "FlavorQCD", "HF", "HF_2018", "RelativeBal", "RelativeSample_2018"};//, "Total"};
    //string jec_source_N[12] = {"_jec_source_0", "_jec_source_1", "_jec_source_2", "_jec_source_3", "_jec_source_4", "_jec_source_5", "_jec_source_6", "_jec_source_7", "_jec_source_8", "_jec_source_9", "_jec_source_10", "_jec_source_11"}; 
    const int nsrc = jecAK4chs_source_.size();//12;
    std::vector<JetCorrectionUncertainty*> vsrc;
    vsrc.reserve(nsrc);
    //JetCorrectorParameters p_total("Summer13_V4_DATA_UncertaintySources_AK5PFchs.txt", "Total");
    //std::cout<<"ok, test"<<std::endl;
    //JetCorrectionUncertainty* total = new JetCorrectionUncertainty(p_total);

    std::cout<<"ok, start jetUser data"<<std::endl;
    //for (int isrc = 0; isrc < 12; isrc++) {
    for (auto i1src=jecAK4chs_source_.begin(); i1src != jecAK4chs_source_.end(); i1src++) {
        //const char *name = srcnames[isrc];
        //std::cout<<*i1src<<std::endl;
        JetCorrectorParameters *p = new JetCorrectorParameters(jecAK4chs_Uncfile_, *i1src);
        //JetCorrectorParameters p(jecAK4chs_Uncfile_, *i1src);
        vsrc.push_back(new JetCorrectionUncertainty(*p));
        delete p;
    } 
    //std::cout<<"ok2, start jetUser data"<<std::endl;
    ...
    //std::cout<<"ok3, start jetUser data"<<std::endl;
...
/*    std::cout<<"okt1, start jetUser data"<<std::endl;

    const int nsrc = 11;
    std::vector<JetCorrectionUncertainty*> vsrc;
    vsrc.reserve(nsrc);
    std::cout<<"okt2, start jetUser data"<<std::endl;
    //JetCorrectorParameters *p = new JetCorrectorParameters(jecAK4c
    JetCorrectorParameters *p_total= new JetCorrectorParameters("RegroupedV2_Autumn18_V19_MC_UncertaintySources_AK4PFchs.txt", "Total");
    std::cout<<"ok, test"<<std::endl;
    ...
    for (int isrc = 0; isrc < 10; isrc++) {
        ...
        JetCorrectorParameters p("RegroupedV2_Autumn18_V19_MC_UncertaintySources_AK4PFchs.txt", name);
        vsrc.push_back(new JetCorrectionUncertainty(p));
    }
    std::cout<<"ok2, start jetUser data"<<std::endl;
*/
...
//std::cout<<jetCorrFactor<<" "<<rawJetP4.pt()<<std::endl;
//std::cout<<"before JER : "<<smearedP4_JEC_up.pt()<<" "<<smearedP4_JEC_down.pt()<<endl;
...
//std::cout<<"JER par up : "<<get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor)<<std::endl;
		smearedP4_JEC_up   *= get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
//std::cout<<"JER par up : "<<get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor)<<std::endl;
//std::cout<<"JER par down : "<<get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor)<<std::endl;
		smearedP4_JEC_down *= get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
//std::cout<<"JER par down : "<<get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor)<<std::endl;

//std::cout<<"JER : "<<smearedP4_JEC_up.pt()<<" "<<smearedP4_JEC_down.pt()<<endl;

        reco::Candidate::LorentzVector tem_smearedP4_abs_up_raw, tem_smearedP4_abs_down_raw;
        tem_smearedP4_abs_up_raw = smearedP4_JEC_up;
        tem_smearedP4_abs_down_raw = smearedP4_JEC_down;
//std::cout<<tem_smearedP4_abs_up_raw.pt()<<"  "<<tem_smearedP4_abs_down_raw.pt()<<std::endl;
...
//std::cout<<"new:"<<tem_smearedP4_abs_up_raw.pt()<<"  "<<tem_smearedP4_abs_down_raw.pt()<<std::endl;

        std::vector<reco::Candidate::LorentzVector> smearedP4_abs_up_raw,smearedP4_abs_down_raw;

    for (int isrc = 0; isrc < nsrc; isrc++) {
        //std::vector<reco::Candidate::LorentzVector> smearedP4_abs_up_raw,smearedP4_abs_down_raw;

        JetCorrectionUncertainty *unc = vsrc[isrc];
        unc->setJetPt(tem_smearedP4_abs_up_raw.pt());
        //unc->setJetEta(tem_smearedP4_abs_up_raw.eta());
        unc->setJetEta(jet.eta());
        double sup = unc->getUncertainty(true); // up variation
        unc->setJetPt(tem_smearedP4_abs_down_raw.pt());
        //unc->setJetEta(tem_smearedP4_abs_down_raw.eta());
        unc->setJetEta(jet.eta());
        double sdw = unc->getUncertainty(false); // down variation


        smearedP4_abs_up_raw.push_back(tem_smearedP4_abs_up_raw*(1+sup));
        smearedP4_abs_down_raw.push_back(tem_smearedP4_abs_down_raw*(1-sdw));
        if(jecAK4chs_source_[isrc] == "Absolute_2018" || jecAK4chs_source_[isrc] == "Absolute"){
//            std::cout<<jecAK4chs_source_[isrc]<<" : "<<std::endl
//            <<1-sdw<<" : 1 : 1 : " <<1+sup<<std::endl
//            <<smearedP4_abs_down_raw[isrc].pt()<<" : "<<tem_smearedP4_abs_down_raw.pt()<<" : "<<tem_smearedP4_abs_up_raw.pt()<<" : "<<smearedP4_abs_up_raw[isrc].pt()<<std::endl
//            <<smearedP4_abs_down_raw[isrc].eta()<<" : "<<tem_smearedP4_abs_down_raw.eta()<<" : "<<tem_smearedP4_abs_up_raw.eta()<<" : "<<smearedP4_abs_up_raw[isrc].eta()<<std::endl;
        }
		jet.addUserFloat(jecAK4chs_source_[isrc]+"_SmearedPt_JEC_up",    smearedP4_abs_up_raw[isrc].pt());
		jet.addUserFloat(jecAK4chs_source_[isrc]+"_SmearedE_JEC_up",     smearedP4_abs_up_raw[isrc].energy());
		jet.addUserFloat(jecAK4chs_source_[isrc]+"_SmearedPt_JEC_down",    smearedP4_abs_down_raw[isrc].pt());
		jet.addUserFloat(jecAK4chs_source_[isrc]+"_SmearedE_JEC_down",     smearedP4_abs_down_raw[isrc].energy());

        // MET
        unc->setJetPt(jetCorrFactor_l1*rawJetP4.pt());
        unc->setJetEta(jet.eta());
        double sup_l1 = unc->getUncertainty(true); // up variation
        unc->setJetPt(jetCorrFactor_l1*rawJetP4.pt());
        unc->setJetEta(jet.eta());
        double sdw_l1 = unc->getUncertainty(false); // down variation


        reco::Candidate::LorentzVector tem_rawJetP4_MET = rawJetP4_MET;
		...
		if ( !(skipEM_ && emEnergyFraction > skipEMfractionThreshold_) ){
			if ( skipMuons_ ) {
				...
				for ( std::vector<reco::CandidatePtr>::const_iterator cand = cands.begin();
					...
					if ( mu != 0 && (*skipMuonSelection_)(*mu) ) {
						...
						tem_rawJetP4_MET -= muonP4;
					}
				}
			}
			reco::Candidate::LorentzVector corrJetP4_MET = jetCorrFactor*tem_rawJetP4_MET;
			reco::Candidate::LorentzVector corrJetP4_up_MET = jetCorrFactor*(1+sup)*tem_rawJetP4_MET;
			reco::Candidate::LorentzVector corrJetP4_down_MET = jetCorrFactor*(1-sdw)*tem_rawJetP4_MET;
			reco::Candidate::LorentzVector rawJetP4offsetCorr_MET = jetCorrFactor_l1*tem_rawJetP4_MET;
			reco::Candidate::LorentzVector rawJetP4offsetCorr_up_MET = jetCorrFactor_l1*(1+sup_l1)*tem_rawJetP4_MET;
			reco::Candidate::LorentzVector rawJetP4offsetCorr_down_MET = jetCorrFactor_l1*(1-sdw_l1)*tem_rawJetP4_MET;
			if ( corrJetP4_MET.pt() > type1JetPtThreshold_ ) {
                ...
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrEx_MET_JEC",     corrEx_MET_JEC);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrEy_MET_JEC",     corrEy_MET_JEC);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrSumEt_MET_JEC",     corrSumEt_MET_JEC);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrEx_MET_JEC_up",     corrEx_MET_JEC_up);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrEy_MET_JEC_up",     corrEy_MET_JEC_up);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrSumEt_MET_JEC_up",     corrSumEt_MET_JEC_up);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrEx_MET_JEC_down",     corrEx_MET_JEC_down);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrEy_MET_JEC_down",     corrEy_MET_JEC_down);
                jet.addUserFloat(jecAK4chs_source_[isrc]+"_corrSumEt_MET_JEC_down",     corrSumEt_MET_JEC_down);
    } // for isrc
